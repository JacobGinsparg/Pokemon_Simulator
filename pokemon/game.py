...
class Game:
    ...
    def create_game(cls, team_path_1, team_path_2):
        ...
        id_2 = str(uuid.uuid4())
        game.players = {}
        game.players[id_1] = _team_from_file_path(id_1, team_path_1)
        game.players[id_2] = _team_from_file_path(id_2, team_path_2)
        game.field_effects = []
        game.weather = None
    ...
    def prompt_player_actions(self):
        for player in self.players:
            ...
            while player_choice is None:
                self.print_player_choices(player)
                choice = input('Your choice: ')
                player_choice = self.get_choice_for_player(player, choice)
            self.player_actions.append(player_choice)

    def print_player_choices(self, player):
    ...
    def get_choice_for_player(self, player, choice_string):
        available = self.get_player_available_pokemon(player)
        active = self.get_player_active_pokemon(player)
        non_active = [poke for poke in available if poke.id is not active.id]
        moves = active.get_moves()
        if choice_string in moves: # Using a move
            return ('move', active.id, choice_string)
        elif choice_string in non_active: # Switching out
    ...
    def perform_player_actions(self):
        for action in self.player_actions:
            ...
            if action[0] is 'move':
                ...
                name = action[2]
                self.players[team_id]['active_id'] = [poke.id for poke in available if poke.name is name][0]
            else:
